...
* 目录结构
  ...
  * `home`：所有用户的主目录，每个用户都有一个自己的目录，一般该目录是以用户的账户命名的。home里面的都是普通用户，没有root用户。
...
* 使用mv命令，既可以移动文件，也可以给文件改名

* 改名字： `mv laowang wangfei`
* 移动文件：`mv wangfei mytest` 
  * 把当前文件移动到上一级目录：`mv hello.c ../`

## 软链接的创建

* 创建软链接就相当于创建快捷方式
* 给**文件**创建软链接和给**目录**创建软链接的快捷方式是一样的。
* 通用命令：`ln -s 路径 快捷文件名`
* 命令示例：`ln -s hello.c hello.soft`
* 你打开hello.soft和打印hello.c其实是一样的
* 但是此时如果你`mv hello.c ../`，把源文件移动到上一级目录，此时hello.soft就会背景加红色，提示没有源文件或目录了。也就是说我们创建了快捷方式以后，源文件就不能移动位置了。
* 我们如何让创建的快捷方式在整个Linux操作系统中都能够使用呢？我们在创建软链接的时候需要创建一个绝对路径。
  * `ln -s /root/liming/hello.c hello.soft`
* 所以我们创建软链接的时候，最好写绝对路径。
...
* 硬链接：将多个目录项中的文件指针指向同一个innode结点，它不占用磁盘空间

* 硬链接就不是快捷方式了，相当于给原来的文件做了一个备份。
* 这个硬链接只能作用于文件，不能作用于目录。
* 命令：`ln test test.hard`——.hard只是自己作为标识，写不写都行

## 搜集的资料

* [Linux中软硬连接的区别](https://zhuanlan.zhihu.com/p/500210522)
* [linux中软链接和硬链接是什么意思？](https://www.zhihu.com/question/443459832/answer/1723706381)
...
* `chmod`命令 – 改变文件或目录权限
  ...
  * **文字设定法**：chmod [who\] [+|-|=] \[mod]
    ...
    * mod:
      ...
      * x:执行
    * 举例：
      * `chmod o+w temp`表示为temp文件的其他人添加写权限
      * `chmod o+w temp`表示为temp文件所有人添加写权限
  * 数字设定法——用的比较多，因为比较简单：
    * -：没有权限
    * r:4
    * w:2
    * x:1
    *  举例：
      * `chmod 755 755`  ——给temp所有者读写执行权限，所属组读，执行权限，其他人读，执行权限
      * `chmod -001 temp`——给temp其他人减去执行权限
